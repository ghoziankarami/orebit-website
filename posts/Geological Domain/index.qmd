...
# Ensure all lithologies have colors
unique_lithos <- unique(combined_data$lithology)
if(length(litho_colors) < length(unique_lithos)) {
  missing_lithos <- setdiff(unique_lithos, names(litho_colors))
  new_colors <- RColorBrewer::brewer.pal(length(missing_lithos), "Set2")
  litho_colors <- c(litho_colors, setNames(new_colors, missing_lithos))
}
